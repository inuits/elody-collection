...
from filters.matchers.matchers import ExactMatcher
...
class BaseFilterType(ABC):
    ...
    @abstractmethod
    def generate_query(self, filter_request: dict):
        pass
...
class TextFilterType(BaseFilterType):
    def __init__(self):
        super().__init__()
        self.matchers.update({"exact_matcher": ExactMatcher})

    def generate_query(self, filter_request_body: dict):
        self.filter_type_engine.generate_query_for_text_filter_type(
            self.matchers, filter_request_body
        )

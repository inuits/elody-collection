...
class Documents(GenericObjectV2):
    ...
    def get(self, *, spec, collection="", is_type_required=False):
        if "export" not in get_features().get("bulk_operations", {}):
        ...
        try:
            ...
            for content in contents:
                ...
                try:
                    ...
                    raise BadRequest(
                        message
                    ) from exception  # temporary BadRequest until error codes are reworked
                if enable_jobs and (
                    current_job_id := (
    ...
    def delete(self, *, spec):
        ...
        if (
            "delete" not in get_features().get("bulk_operations", {})
        ...
        original_soft = request.args.get("soft")
        ...
        for id in identifiers:
            try:
                response = document_resource.delete(id=id, spec=spec)
            except Forbidden:
                response = id, 403
                forbidden_ids.append(id)
                disallow_delete = "forbidden"
            except Unauthorized:
                response = id, 401
                disallow_delete = "unauthorized"
            soft_responses.append(response)

        match disallow_delete:
            case "forbidden":
                fail_job(
                    job_id,
                    exception_message=f"User does not have permission to delete entities: {forbidden_ids}",
                    get_rabbit=get_rabbit,
                )
                raise Forbidden
            case "unauthorized":
        ...
        if original_soft != "1":
            new_args["soft"] = "0"
            request.args = ImmutableMultiDict(new_args)
            user_id = get_user_context().id
            new_headers = Headers(request.headers)
            new_headers["Authorization"] = f"Bearer {getenv('STATIC_JWT')}"
            new_headers.pop("X-User-Email", None)
            new_headers["X-User-Email"] = user_id

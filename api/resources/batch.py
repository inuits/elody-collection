        ...
        if content_type == "text/csv":
            ...
            if accept_header == "application/json":
                ...
                for mediafile in entities_and_mediafiles.get("mediafiles", list()):
                    ...
                    link = f"{self.storage_api_url_for_caller}/upload-with-ticket/{quote(filename)}?id={get_raw_id(mediafile)}&ticket_id={ticket_id}"
                    output["links"].append(link)
            else:
                output = ""
                for mediafile in entities_and_mediafiles.get("mediafiles", list()):
                    ticket_id = self._create_ticket(mediafile.get("filename"))
                    output += f"{self.storage_api_url_for_caller}/upload-with-ticket/{quote(mediafile.get('filename'))}?id={get_raw_id(mediafile)}&ticket_id={ticket_id}\n"
            finish_job(self.job_id_with_validation, get_rabbit=self.get_rabbit)

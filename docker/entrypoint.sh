...
if [ "$APP_ENV" = "dev" ]; then
  ...
  cd ~/api
  exec ~/.local/bin/gunicorn -b 0.0.0.0 --timeout 0 $GUNICORN_CMD_ARGS "app:app" --keep-alive 30 --access-logfile - --access-logformat '%(h)s %(l)s %(u)s %(t)s "%(r)s" %(s)s %(M)sms %(b)s "%(f)s" "%(a)s"'
fi
